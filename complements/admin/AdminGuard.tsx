// complements/admin/AdminGuard.tsx
'use client'
...
function AdminGate({
    ...
    async function check() {
      ...
      try {
        ...
        const resp = await fetch(
          `/api/admin/ai-agents/${agentId}?_=${Date.now()}`,
          {
        ...
        const roleFromServer: AclRole | null =
          parsed && typeof parsed.role === 'string'
            ? (parsed.role as AclRole)
            : null
        ...
        if (resp.ok) {
          ...
          setError(
            (parsed && typeof parsed.error === 'string'
              ? parsed.error
              : text) || 'Permisos insuficientes o token inválido.',
          )
        }
      } catch (e: any) {
        if (cancel) return
      ...
      <Wrapper>
        <Card>
          ...
          <P className="text-sm text-white/70">
            <FM
              id="loadingSession"
              defaultMessage="Cargando sesión…"
            />
          </P>
        </Card>
      </Wrapper>
        ...
        <Card>
          ...
          <div className="flex items-center gap-3">
            <BUTTON onClick={handleLogin}>
              <FM
                id="loginWithGoogle"
                defaultMessage="Acceso de Controlador"
              />
            </BUTTON>
          </div>
          <Divider />
          <LINK
            href="/"
            className="rounded-full border border-white/20 p-3.5 text-xs text-white/70 hover:bg-white/30"
          >
            <FM
              id="backtohome"
          ...
          <P className="text-xs text-white/50">
            <FM
              id="restrictedPanel"
              defaultMessage="Panel restringido a cuentas autorizadas."
            />
          </P>
        </Card>
      </Wrapper>
    ...
    return (
      <Wrapper>
        <Card>
          ...
          <P className="text-sm text-white/70">
            <FM
              id="checkingPermissions"
              defaultMessage="Verificando permisos…"
            />
          </P>
          <Divider />
  ...
  return (
    <AuthContext.Provider
      value={{ user, idToken, allowed, checking, role }}
    >
      <div className="sticky top-0 z-10 flex items-center justify-between gap-3 p-3 border-b border-white/10 bg-black/50 backdrop-blur">
        ...
        <LINK
          href="/admin"
          className="bg-white text-black pr-2 pl-2 rounded-md"
        >
